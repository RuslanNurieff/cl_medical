...
class Trainer:
    ...
    def __init__(
        ...
        if early_stopper is not None:
            eval_dataset, val_dataset = self.create_validation_set(eval_dataset, 0.9)
    ...
    def train(self):
        ...
        for epoch in range(self.train_args.epochs):
            ...
            if self.early_stopper is not None and self.val_dataloader is not None:
                    val_result = Evaluator.evaluate(self.model, self.val_dataloader, [METRIC_MAPPING[self.early_stop_metric]], self.device,)
                    val_metric = list(val_result.values())[0]

                    if self.logger is not None:
                        self.logger.log({
                            f"{self.logging_prefix}val/{metric_name}": value for metric_name, value in val_result.items()
                        })
